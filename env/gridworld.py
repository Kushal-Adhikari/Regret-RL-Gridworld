...
class Grid_World:
    ...
    def reset(self):
        self.positions = torch.zeroes( (self.batch_size , 2), dtype= torch.int64 , device = self.device)
        self.done = torch.zeroes((self.bath_size , 2) , dtype = bool , device = self.device)
    ...
    def step( self, actions):
        ...
        actual_actions = torch.where(rand < self.Stochastic_probabilities , random_actions , actions)
